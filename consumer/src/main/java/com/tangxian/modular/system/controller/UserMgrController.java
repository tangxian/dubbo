...
import com.tangxian.config.properties.GunsProperties;
...
import com.tangxian.core.util.DateHelper;
import com.tangxian.modular.system.factory.UserFactory;
import com.tangxian.modular.system.model.User;
import com.tangxian.modular.system.service.IUserService;
import com.tangxian.modular.system.transfer.UserDto;
import com.tangxian.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    @ResponseBody
    public ResponseData register_user(@Valid User user) {
        // 判断账号是否重复
        User user_account = userService.getByAccount(user.getAccount());
        if (user_account != null) {
            throw new ServiceException(BizExceptionEnum.USER_ALREADY_REG);
        }
        //判断邮箱是否重复
        User user_email = userService.getByEmail(user.getEmail());
        if (user_email != null) {
    ...
    public ResponseData edit(@Valid UserDto user, BindingResult result) throws NoPermissionException {
        ...
        //判断用户账号是否存在
        int account = userService.selectCount(new EntityWrapper<User>().eq("account", user.getAccount()));
        if (!oldUser.getAccount().equals(user.getAccount())) { //用户修改用户名
            if (account > 0) {
                throw new ServiceException(BizExceptionEnum.USER_ALREADY_REG);
            }
        }
     ...
     * 生成短信验证码
     */
    @RequestMapping(value = "/generateVerificationCode")
    @ResponseBody
    public Object generateVerificationCode(String phonenum) {
        JSONObject obj = new JSONObject();
        String verifyCode = constantFactory.generateVerifyCode(phonenum);
        obj.put("verificationcode", verifyCode);
        return obj;
    }
     ...
     * 传入手机号获取缓存中短信验证码
     */
    @RequestMapping(value = "/getVerificationCodeByPhonenum")
    @ResponseBody
    public Object getVerificationCodeByPhonenum(String phonenum, String verificationcode) {
        JSONObject obj = new JSONObject();
        String verifyCode = constantFactory.getVerifyCode(phonenum);

...
Resource.initColumn = function () {
    return [
        ...
        {
            title: '出版社', field: 'publish', visible: true, align: 'center', valign: 'middle',width:'80px',
            formatter: function (value, row, index) {
                return "<span style='display: block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;' title='" + value + "'>" + value + "</span>";
            }
        },
        ...
        {title: '录入人', field: 'userName', visible: true, align: 'center', valign: 'middle',width:'80px'},
        {
            ...
            formatter: function (value, row, index, field) {
                return [
                    ...
                    '<button type="button" onclick="Resource.openResourceDownAway(' + row["id"] + ')" class="RoleOfedit btn btn-primary  btn-sm" style="margin-right:15px;    margin-bottom: 0px;">下架</button>'
                ].join('');
            }
        },
    ];
 ...
 * 资源下架
 */
Resource.openResourceDownAway = function (id) {
    var ajax = new $ax(Feng.ctxPath + "/resource/downaway_save", function (data) {
        Feng.success("下架成功!");
        window.location.reload();
    }, function (data) {
        Feng.error("操作失败!" + data.responseJSON.message + "!");
    });
    ajax.set("resourceId", id);
    ajax.start();
};
...
Resource.openResourceUpdate = function (id) {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/resource/resource_has_update/' + id
    });
    this.layerIndex = index;
};
